/**
 * Report_DailyDetailed.gs — детальне зведення дня (графік + PERSONNEL + DICT_SUM).
 */

/** Рядки місячного графіка з кодом і позивним на обрану дату. */
function getMonthlyScheduleEntriesForColumn_(sheet, col) {
  ...
  for (let i = 0; i < codes.length; i++) {
    ...
    if (code && rowCallsign) {
      entries.push({ rowIndex: startRow + i, code: code, callsign: rowCallsign });
    }
  }
  return entries;
}
...
function countMonthlyScheduleRowsForColumn_(sheet, col) {
  return getMonthlyScheduleEntriesForColumn_(sheet, col).length;
}
...
function collectPeopleDetailed_(sheet, col) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    seen.add(key);
    people.push({ code: entry.code, fullName: fml, surname: surname });
  }
  return people;
}
...
function formatDetailedSummary_(date, people) {
  try {
    ...
    const all = new Set(people.map(function (p) {
      return p.surname;
    }));
    ...
    for (let i = 0; i < groupRules.length; i++) {
      ...
      const subset = people.filter(function (p) {
        return codes.indexOf(p.code) !== -1 && !usedSurnames.has(p.surname);
      });
      const set = new Set(
      ...
      const namesList =
        set.size > 0 ? Array.from(set).sort().join(", ") : "—";
      if (groupTemplate) {
...
function formatDetailedSummaryLegacy_(date, people) {
  ...
  readDictSum_()
    ...
    .forEach(function (rule) {
      ...
      txt +=
        "*" +
        (rule.label || FULL_NAMES[group] || group) +
        "* — " +
        set.size +
        "\n" +
  ...
  return preserveUserConditionalFormatRules_(
    ...
    function () {
      ...
      const groupOf = function (code) {
        const entries = Object.entries(SUMMARY_GROUPS || {});
        for (let i = 0; i < entries.length; i++) {
          if (entries[i][1].indexOf(code) !== -1) return entries[i][0];
        }
        return "Інше";
      };
  ...
  try {
    ...
    if (!phone) {
      ...
      ui.alert(
        "✕ Телефон не знайдено",
        'Для ролі "' +
          (recipient.role || CONFIG.COMMANDER_ROLE) +
          '" не знайдено телефону.\n\n' +
          "Перевірте:\n" +
          '1. В аркуші PHONES є запис з роллю "' +
          (recipient.role || CONFIG.COMMANDER_ROLE) +
          '" в колонці C\n' +
