...
## Security hardening
- RBAC for maintenance/admin actions via `ACCESS` sheet or Script Properties.
- LOG/AUDIT sanitization for phone/message/link fields.
- Retention cleanup now includes `LOG` and `AUDIT_LOG`.
- WhatsApp links use `_blank` for HTML Service IFRAME compatibility.
- Runtime backoff + `ALERTS_LOG` for repeated job failures.
