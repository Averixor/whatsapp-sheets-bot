function runStage3HealthCheck_(options) {
  ...
  Object.keys(schemas).forEach(function (key) {
    ...
    if (schema.key === "MONTHLY") {
      _stage7PushCheck_(
        ...
        sheet ? "OK" : "FAIL",
        sheet
          ? `Активний місячний лист: ${sheetName}`
  ...
  [
    ...
    _stage7PushCheck_(
      ...
      exists ? "" : "Перевірте файл stage 7 repository",
    );
  });
  ...
  try {
    ...
    contractChecks.forEach(function (result, idx) {
      ...
      _stage7PushCheck_(
        ...
        valid ? "OK" : "FAIL",
        valid
          ? "Контракт відповіді валідний"
          : "Відповідь не відповідає server-side contract",
...
function runStage4HealthCheck_(options) {
  ...
  const includeReconciliationPreview =
    opts.includeReconciliationPreview !== false;
  ...
  const checks = [].concat(base.checks || []).filter(function (item) {
  ...
  [
    ...
    const exists = !!(
      Stage7UseCases_ && typeof Stage7UseCases_[name] === "function"
    );
    _stage7PushCheck_(
      checks,
  ...
  try {
    ...
    _stage7PushCheck_(
      ...
      auditSheet ? "OK" : "WARN",
      auditSheet
        ? `Аркуш "${auditSheet.getName()}" доступний`
        : "AUDIT_LOG не створений",
  ...
  if (!shallow && includeReconciliationPreview) {
    try {
      ...
      const reconciliationIssues = Number(
        (reconciliation &&
          reconciliation.summary &&
          reconciliation.summary.issues) ||
          0,
      );
      const reconciliationCritical = Number(
        (reconciliation && reconciliation.criticalCount) || 0,
      );
      const reconciliationMessage =
        reconciliation.message ||
        "Reconciliation preview: проблем " +
...
function runStage5MetadataConsistencyCheck_() {
  ...
  const maintenancePolicy =
    typeof getMaintenancePolicy_ === "function"
      ? getMaintenancePolicy_()
      : (meta && meta.maintenanceLayerPolicy) || {};
  const release =
  ...
  _stage7PushCheck_(
    ...
    meta.activeBaseline === "stage7-1-5-maintenance-baseline" ? "OK" : "FAIL",
    `activeBaseline=${meta.activeBaseline || "n/a"}`,
    "Зафіксуйте Stage 7.1 як active baseline",
  );
  _stage7PushCheck_(
    checks,
    "Release archive naming",
    release && release.archiveFileName === "gas_wasb_stage7_1_5_maintenance.zip"
      ? "OK"
      : "FAIL",
  ...
  _stage7PushCheck_(
    ...
    release && release.rootFolderName === "gas_wasb_stage7_1_5_maintenance"
      ? "OK"
      : "FAIL",
  ...
  _stage7PushCheck_(
    checks,
    "Packaging policy marker",
    meta.packagingPolicy &&
      meta.packagingPolicy.policy === "root-manifest-web-editor-only"
      ? "OK"
      : "FAIL",
    meta.packagingPolicy && meta.packagingPolicy.policy
      ? meta.packagingPolicy.policy
      : "Не задано",
    "Зафіксуйте web-editor-only packaging policy",
  );
  _stage7PushCheck_(
    checks,
    "Root manifest declared",
    meta.manifestIncluded === true ? "OK" : "FAIL",
    `manifestIncluded=${meta.manifestIncluded}`,
    "Вирівняйте metadata",
  );
  _stage7PushCheck_(
    checks,
    "Root manifest physical",
    _projectBundleHas_(
    ...
    _projectBundleHas_(
      ...
      ? (meta.packagingPolicy && meta.packagingPolicy.manifestPath) ||
          "appsscript.json"
      : "manifest missing",
    "Додайте appsscript.json у root",
  );
  _stage7PushCheck_(
    checks,
  ...
  _stage7PushCheck_(
    checks,
    "Maintenance layer marker",
    meta.maintenanceLayerStatus === "stage7-canonical-maintenance-api"
      ? "OK"
      : "FAIL",
    `maintenanceLayerStatus=${meta.maintenanceLayerStatus || "n/a"}`,
    "Позначте canonical maintenance layer у metadata",
  );
  _stage7PushCheck_(
    checks,
  ...
  _stage7PushCheck_(
    checks,
    "Sunset policy marker",
    meta.sunsetPolicyMarker === "stage7-sunset-governed" ? "OK" : "WARN",
    `sunsetPolicyMarker=${meta.sunsetPolicyMarker || "n/a"}`,
    "Зафіксуйте Stage 7 sunset policy marker",
  );
  ...
  _stage7PushCheck_(
    ...
    maintenancePolicy &&
      maintenancePolicy.canonicalFile === "Stage7MaintenanceApi.gs"
      ? "OK"
      : "FAIL",
    maintenancePolicy && maintenancePolicy.canonicalFile
      ? maintenancePolicy.canonicalFile
      : "Не задано",
    "Оновіть maintenance policy",
  );
  _stage7PushCheck_(
    checks,
  ...
  _stage7PushCheck_(
    checks,
    "Client runtime file",
    clientRuntimePolicy.runtimeFile === "JavaScript.html" ? "OK" : "FAIL",
    clientRuntimePolicy.runtimeFile || "Не задано",
    "Зафіксуйте JavaScript.html як canonical runtime",
  );
  _stage7PushCheck_(
    checks,
    "Client bootstrap mode",
    clientRuntimePolicy.bootstrapMode === "sidebar-includeTemplate"
      ? "OK"
      : "FAIL",
    clientRuntimePolicy.bootstrapMode || "Не задано",
    "Використовуйте Sidebar.html -> includeTemplate('JavaScript')",
  );
  _stage7PushCheck_(
    checks,
    "Client modular status",
    clientRuntimePolicy.modularStatus === "active-js-include-chain"
      ? "OK"
      : "WARN",
    clientRuntimePolicy.modularStatus || "Не задано",
    "Позначте Js.*.html як non-active experimental/reference artifacts",
  );
  _stage7PushCheck_(
    checks,
    "Diagnostics wording policy",
    meta.diagnosticsPolicy &&
      meta.diagnosticsPolicy.wording === "stage7-1-5-maintenance-baseline"
      ? "OK"
      : "WARN",
    meta.diagnosticsPolicy && meta.diagnosticsPolicy.wording
      ? meta.diagnosticsPolicy.wording
      : "Не задано",
    "Зафіксуйте Stage 7.1 diagnostics wording policy",
  );
  ...
  activeDocs.forEach(function (doc) {
    _stage7PushCheck_(
      checks,
      `Active doc path ${doc}`,
      String(doc || "").indexOf("_extras/") !== 0 ? "OK" : "FAIL",
      doc,
      "Active docs мають лежати в корені bundle",
    );
    _stage7PushCheck_(
      checks,
      `Active doc physical ${doc}`,
      _projectBundleHas_(doc) ? "OK" : "FAIL",
      _projectBundleHas_(doc) ? `present=${doc}` : `missing=${doc}`,
      "Вирівняйте bundle layout",
    );
  ...
  historicalDocs.forEach(function (doc) {
    _stage7PushCheck_(
      checks,
      `Historical doc path ${doc}`,
      String(doc || "").indexOf("_extras/history/") === 0 ? "OK" : "FAIL",
      doc,
      "Historical docs мають лежати в _extras/history/",
    );
  });
  ...
  [
    ...
    _stage7PushCheck_(
      checks,
      `Canonical reference doc ${doc}`,
      _projectBundleHas_(doc) ? "OK" : "FAIL",
      _projectBundleHas_(doc) ? `present=${doc}` : `missing=${doc}`,
      "Відновіть canonical docs у root",
    );
  });
...
function runSunsetDiagnostics_(options) {
  ...
  return _diagBuildReport_(
    _diagNormalizeReportChecks_({ checks: checks }),
    opts.mode || "compatibility sunset",
...
function runFullVerboseDiagnostics_(options) {
  ...
  return _diagBuildReport_(
    ...
    "full-verbose",
    base.ok && hardening.ok
      ? _releaseStageLabel_() + " verbose diagnostics OK"
      : _releaseStageLabel_() + " verbose diagnostics потребують уваги",
  );
}
