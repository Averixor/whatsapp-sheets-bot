...
Одноразова ініціалізація / міграція: сайдбар **Налаштувати облік майна**
(`apiStage7SetupTemporaryPropertyRegister()`), або alias у GAS editor:

```javascript
apiSetupTemporaryPropertyRegister()
```

Безпечне повторне оновлення валідацій: сайдбар **Оновити облік майна**
(`apiStage7RefreshTemporaryPropertyRegister()` / alias `apiRefreshTemporaryPropertyRegister()`).
Верхнє меню spreadsheet — лише **WASB → Відкрити панель**.
