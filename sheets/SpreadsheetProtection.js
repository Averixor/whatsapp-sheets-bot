...
function _spGetConfigValue_(container, key, fallback) {
  try {
    if (
      _spHasObject_(container) &&
      typeof container[key] !== "undefined" &&
      container[key] !== null &&
      container[key] !== ""
    ) {
...
function _spResolveSheetNames_(options) {
  ...
  const config =
    typeof CONFIG !== "undefined" && _spHasObject_(CONFIG) ? CONFIG : {};
  const stage7 =
    typeof STAGE7_CONFIG !== "undefined" && _spHasObject_(STAGE7_CONFIG)
      ? STAGE7_CONFIG
      : {};

  const includeSendPanel =
    typeof opts.includeSendPanel === "boolean" ? opts.includeSendPanel : true;
...
function _spResolveAdminEmails_() {
  ...
  try {
    if (
      _spHasObject_(scope.AccessControl_) &&
      _spHasFunction_(scope.AccessControl_.listAdminEmails)
    ) {
      const emails = scope.AccessControl_.listAdminEmails();
      const normalized = _spUniqueStrings_(
...
function _spResolveFallbackEditor_() {
  try {
    if (
      ...
      _spHasFunction_(Session.getEffectiveUser)
    ) {
...
function _spRemoveDisallowedEditors_(
  protection,
  allowEditors,
  summary,
  sheetName,
  ...
  try {
    ...
    for (let i = 0; i < currentEditors.length; i++) {
      ...
      const email = _spNormalizeEmail_(
        user && _spHasFunction_(user.getEmail) ? user.getEmail() : "",
      );
      if (!email) continue;
    ...
    if (removable.length && _spCanCall_(protection, "removeEditors")) {
      ...
      summary.removedEditors[sheetName] = removable
        .map(function (user) {
          return _spNormalizeEmail_(
            user && _spHasFunction_(user.getEmail) ? user.getEmail() : "",
          );
        })
        .filter(Boolean);
    ...
    summary.warnings.push(
      "Не вдалося видалити зайвих редакторів для листа " +
        sheetName +
        ": " +
        (error && error.message ? error.message : error),
    );
...
function _spAddAllowedEditors_(protection, allowEditors, summary, sheetName) {
  try {
    ...
    summary.warnings.push(
      "Не вдалося додати дозволених редакторів для листа " +
        sheetName +
        ": " +
        (error && error.message ? error.message : error),
    );
...
function _spDisableDomainEdit_(protection, summary, sheetName) {
  try {
    if (
      _spCanCall_(protection, "canDomainEdit") &&
    ...
    summary.warnings.push(
      "Не вдалося вимкнути domain edit для листа " +
        sheetName +
        ": " +
        (error && error.message ? error.message : error),
    );
...
function _spProtectSheet_(
  ...
  const sheetName = _spSafeString_(
    sheet && _spHasFunction_(sheet.getName) ? sheet.getName() : "",
    "",
  );
  ...
  try {
    ...
    if (!protection) {
      summary.warnings.push(
        "Не вдалося створити protection для листа " + sheetName,
      );
      return false;
    }
...
function applySpreadsheetProtections_(options) {
  ...
  try {
    ...
    summary.warnings.push(
      "Не вдалося отримати активну таблицю: " +
        (error && error.message ? error.message : error),
    );
    return summary;
  }
  ...
  const allowEditors = _spUniqueStrings_(
    [fallbackEditor].concat(adminEmails).map(function (email) {
      return _spNormalizeEmail_(email);
    }),
  );
  ...
  for (let i = 0; i < sheetNames.length; i++) {
    ...
    var accessSheetName = _spGetCoreValue_("ACCESS_SHEET", "ACCESS");
    var sheetWarningOnly = summary.warningOnly || sheetName === accessSheetName;
    if (sheetName === accessSheetName && !summary.warningOnly) {
      summary.accessWarningOnly = true;
    }
    ...
    _spProtectSheet_(
      ...
      sheetWarningOnly,
      summary,
      _spSafeString_(opts.description, "WASB service sheet protection"),
    );
  }

  if (summary.warningOnly) {
    summary.warnings.push(
      "Адміністратори не налаштовані або email поточного редактора недоступний — застосовано warningOnly protection.",
    );
  }
  if (summary.accessWarningOnly) {
    summary.warnings.push(
      "ACCESS: warningOnly — реєстрація нових користувачів через Sidebar потребує запису в ACCESS; блокуючий захист вимкнено, аудит onEdit лишається.",
    );
  }
