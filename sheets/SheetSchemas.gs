...
function _ssBuildMonthlySchema_() {
  ...
  return _ssFreeze_({
    ...
    osFmlRangeA1: _ssTrimmedString_(
      _ssConfigValue_("OS_FML_RANGE_A1", "G2:G40"),
      "G2:G40",
    ),
    ...
    notes:
      "Робочий графік: ідентифікатор/позивний рядка, БР і щоденні коди. Персональні дані читаються з PERSONNEL.",
  });
}
...
function _ssBuildPersonnelSchema_() {
  return _ssFreeze_({
    ...
    name: _ssTrimmedString_(
      _ssConfigValue_("PERSONNEL_SHEET", "PERSONNEL"),
      "PERSONNEL",
    ),
    headerRow: 1,
    dataStartRow: 2,
...
function validateSheetHeadersBySchema_(sheet, schemaOrKey) {
  ...
  if (schema.headerBased) {
    ...
    var headerBasedLastCol = Math.max(
      Number(sheet.getLastColumn()) || 0,
      getSchemaLastColumn_(schema),
    );
    var headerBasedHeaders =
      headerBasedLastCol > 0
        ? sheet
            .getRange(headerBasedRow, 1, 1, headerBasedLastCol)
            .getDisplayValues()[0]
        : [];
    var seenHeaders = {};
    ...
    (schema.optionalHeaders || []).forEach(function (headerName) {
      if (seenHeaders[headerName] === undefined) {
        report.warnings.push(
          "Опційний header відсутній або не розпізнаний: " + headerName,
        );
      }
    });
