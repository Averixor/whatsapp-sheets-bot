...
function wasbNormalizeAllSheetHeadersToEnglish() {
  ...
  function setHeaders(sheet, row, headers) {
    ...
    try {
      sheet.setFrozenRows(Math.max(sheet.getFrozenRows(), row));
      sheet
        .getRange(row, 1, 1, headers.length)
        .setFontWeight("bold")
  ...
  function run(name, fn) {
    try {
      if (fn()) report.changedSheets.push(name);
      else report.skippedSheets.push(name);
    } catch (e) {
      report.ok = false;
      report.errors.push({
        sheet: name,
  ...
  function schemaHeaders(key) {
    var schema = SheetSchemas_.get(key),
    ...
    Object.keys(fields).forEach(function (k) {
      var c = Number(fields[k].col) || 0;
      if (c > max) max = c;
    });
    var h = new Array(max);
    for (var i = 0; i < max; i++) h[i] = "";
    Object.keys(fields).forEach(function (k) {
      var f = fields[k],
        c = Number(f.col) || 0;
      if (c > 0) h[c - 1] = String(f.label || k);
    });
    return h;
  }
  var access = ss.getSheetByName(
    typeof ACCESS_SHEET !== "undefined" ? ACCESS_SHEET : "ACCESS",
  );
  if (access)
    run("ACCESS", function () {
      var labels =
        typeof _getAccessHeaderDisplayLabels_ === "function"
          ...
          : {};
      var changed = setHeaders(
        access,
        1,
        SHEET_HEADERS.map(function (header) {
          return labels[header] || header;
        }),
      );
      try {
        _applyRoleValidation_(access);
        _applyEmailValidation_(access);
        _applyEnabledValidation_(access);
        _applySelfBindAllowedValidation_(access);
        _applyRegistrationStatusValidation_(access);
      } catch (e) {
        report.errors.push({
          sheet: "ACCESS",
  ...
  ss.getSheets().forEach(function (sh) {
    if (/^\d{2}$/.test(sh.getName()))
      run(sh.getName(), function () {
        var layout =
          typeof detectMonthlyLayoutFromSheet_ === "function"
            ? detectMonthlyLayoutFromSheet_(sh)
            : null;
        return layout && layout.layout === "compact"
          ? setHeaders(sh, 1, ["BRDays", "Callsign"])
          : setHeaders(sh, 1, [
              "Phone",
              "Callsign",
              "Position",
...
function apiStage7NormalizeAllSheetHeadersToEnglish() {
  ...
  return {
    ...
    message:
      result.ok !== false
        ? "All known sheet headers normalized to English"
        : "Header normalization finished with errors",
    result: result,
