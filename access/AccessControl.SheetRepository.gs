...
// ---------------------------------------------------------------------------
// 2. Human-readable header aliases
// ---------------------------------------------------------------------------
...
function _getAccessHeaderDisplayLabels_() {
  return {
    ...
    self_bind_allowed: "Self_bind",
    user_key_current_hash: "Curr_hash",
    user_key_prev_hash: "Prev_hash",
    ...
    request_user_key_hash: "Req_hash",
    request_created_at: "Req_created",
    temporary_password_plain: "Temp_pass",
...
function _getAccessHeaderAliasMap_() {
  ...
  var legacy = {
    ...
    "ім'я, що відображається": "display_name",
    "імя, що відображається": "display_name",
    "ім'я, що показується": "display_name",
    "імя, що показується": "display_name",
    ...
    ким_схвалено: "approved_by",
    approved_by: "approved_by",
    "час схвалення": "approved_at",
    "коли схвалено": "approved_at",
    час_схвалення: "approved_at",
    коли_схвалено: "approved_at",
    approved_at: "approved_at",
...
// ---------------------------------------------------------------------------
// 3. Sheet retrieval & schema enforcement
// ---------------------------------------------------------------------------
...
function _applyEmailValidation_(sh) {
  var col = _getHeaderMap_(sh).email;
  if (!col) return;
...
// ---------------------------------------------------------------------------
// 5. Automatic field synchronisation
// ---------------------------------------------------------------------------
...
function _setEntryField_(sheetRow, header, value) {
  ...
  var col = _getHeaderMap_(sh)[header];
  if (!col) return false;
  sh.getRange(sheetRow, col).setValue(value);
...
function _entryToHeaderUpdates_(entry) {
  var e = entry || {};
  var updates = {};
  ...
  if (e.user_key_prev_hash !== undefined)
    updates.user_key_prev_hash = normalizeStoredHash_(e.user_key_prev_hash);
  if (e.lastSeenAt !== undefined) updates.last_seen_at = e.lastSeenAt;
  if (e.last_seen_at !== undefined) updates.last_seen_at = e.last_seen_at;
  if (e.lastRotatedAt !== undefined) updates.last_rotated_at = e.lastRotatedAt;
  if (e.last_rotated_at !== undefined)
    updates.last_rotated_at = e.last_rotated_at;
  ...
  if (e.patronymic !== undefined)
    updates.patronymic = normalizeHumanName_(e.patronymic);
  ...
  if (e.password_hash !== undefined)
    updates.password_hash = String(e.password_hash || "").trim();
  if (e.password_salt !== undefined)
    updates.password_salt = String(e.password_salt || "").trim();
  if (e.failed_attempts !== undefined)
    updates.failed_attempts = e.failed_attempts;
  if (e.locked_until_ms !== undefined)
    updates.locked_until_ms = e.locked_until_ms;

  return updates;
}

function _writeEntryByHeaderMap_(sheetRow, entry) {
  return _setEntryFields_(sheetRow, _entryToHeaderUpdates_(entry));
}
...
// ---------------------------------------------------------------------------
// 7. Utilities (column lookup, hashing, temporary passwords)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 8. Public API – normalise sheet formatting
// ---------------------------------------------------------------------------
