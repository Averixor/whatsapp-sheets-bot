...
var Stage7TestRunner = (function () {
      ...
      if (opts.writeToLogger) {
        Logger.log(
          "WASB TEST RUN: runId=" +
            ...
            (report.ok ? "OK" : "FAIL") +
            "; passed=" +
            ((report.counts && report.counts.passed) || 0) +
            "; failed=" +
            ((report.counts && report.counts.failed) || 0) +
            "; warnings=" +
            ((report.counts && report.counts.warnings) || 0) +
            "; offset=" +
            (typeof report.offset === "number" ? report.offset : "-") +
            "; nextOffset=" +
            (typeof report.nextOffset === "number" ? report.nextOffset : "-") +
            "; totalTasks=" +
            (report.totalTasks || (report.counts && report.counts.total) || 0) +
            "; done=" +
            (report.done === true),
        );
      }
    ...
    try {
      ...
      if (opts.writeToSheet) {
        try {
          ...
          report.warnings.push(
            "Не вдалося записати звіт у лист: " + getErrorMessage_(sheetError),
          );
        }
      }
    ...
    try {
      ...
      if (!fn) {
        ...
        result.recommendation =
          "Якщо це очікуваний legacy runner — можна залишити. Якщо тест має існувати, перевірити імпорт файлу.";
      ...
      result.status = normalizeCompatibilityStatus_(
        inferStatus_(result.details, task),
        task,
      );
      ...
      result.recommendation = buildRecommendation_(
        result.status,
        result.details,
        task,
      );
  ...
  function inferStatus_(details, task) {
    ...
    if (typeof raw === "object") {
      ...
      if (
        numberGreaterThanZero_(raw.fail) ||
        numberGreaterThanZero_(raw.failed) ||
        numberGreaterThanZero_(raw.failures) ||
        numberGreaterThanZero_(raw.errorCount)
      )
        return "FAIL";
      if (
        arrayHasItems_(raw.failed) ||
        arrayHasItems_(raw.failures) ||
        arrayHasItems_(raw.errors)
      )
        return "FAIL";
      ...
      if (
        numberGreaterThanZero_(raw.warningCount) ||
        numberGreaterThanZero_(raw.warningsCount)
      )
        return "WARN";
      if (arrayHasItems_(raw.warnings)) return "WARN";
      if (arrayHasItems_(raw.issues))
        return task && task.severity === "critical" ? "FAIL" : "WARN";

      if (raw.dataIntegrity && objectHasNonEmptyArrays_(raw.dataIntegrity))
        return task && task.severity === "critical" ? "FAIL" : "WARN";
      if (raw.criticalIssues && arrayHasItems_(raw.criticalIssues))
        return "FAIL";
    }
  ...
  function inferChecksStatus_(items) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (
        arrayHasItems_(item.errors) ||
        arrayHasItems_(item.failed) ||
        arrayHasItems_(item.failures)
      )
        return "FAIL";

      if (item.status && isWarnStatus_(item.status)) hasWarn = true;
      if (item.result && isWarnStatus_(item.result)) hasWarn = true;
      if (arrayHasItems_(item.warnings) || arrayHasItems_(item.issues))
        hasWarn = true;
    }
  ...
  function statusToUiGroup_(status, ok, task) {
    if (status === "SKIPPED")
      return isCompatibilityTask_(task) ? "compatibility" : "pseudo";
  ...
  function detailsObjectToHumanText_(raw, limit) {
    ...
    if (Array.isArray(raw.checks)) {
      return withDetailsTitle_(
        title,
        detailsChecksToHumanText_(raw.checks, limit),
        limit,
      );
    }
    ...
    if (Array.isArray(raw.results)) {
      return withDetailsTitle_(
        title,
        detailsChecksToHumanText_(raw.results, limit),
        limit,
      );
    }
    ...
    if (Array.isArray(raw.errors) && raw.errors.length) {
      return withDetailsTitle_(
        title,
        "Помилки: " + detailsChecksToHumanText_(raw.errors, limit),
        limit,
      );
    }
    ...
    if (Array.isArray(raw.warnings) && raw.warnings.length) {
      return withDetailsTitle_(
        title,
        "Попередження: " + detailsChecksToHumanText_(raw.warnings, limit),
        limit,
      );
    }
  ...
  function buildRecommendation_(status, details, task) {
    ...
    if (raw && typeof raw === "object") {
      if (raw.recommendation)
        return humanizeReportValue_(raw.recommendation, 700);
    ...
    if (status === "SKIPPED" && isCompatibilityTask_(task))
      return "Legacy/historical compatibility. Не блокує Stage 7 deploy; тримати як довідковий технічний борг або запускати окремо.";
    return "";
  }
  ...
  function buildDialogHtml_(report) {
    ...
    var rows = (report.results || [])
      .map(function (item) {
        return (
          "<tr>" +
          "<td>" +
          escapeHtml_(item.group) +
          "</td>" +
          "<td>" +
          escapeHtml_(item.level) +
          "</td>" +
          "<td>" +
          escapeHtml_(item.name) +
          "</td>" +
          "<td>" +
          escapeHtml_(item.functionName) +
          "</td>" +
          '<td class="' +
          escapeHtml_(String(item.status).toLowerCase()) +
          '">' +
          escapeHtml_(item.status) +
          "</td>" +
          "<td>" +
          escapeHtml_(String(item.durationMs)) +
          "</td>" +
          "<td>" +
    ...
    return (
      ...
      escapeHtml_(report.runId) +
      " | Mode: " +
      escapeHtml_(report.mode) +
      " | Started: " +
      escapeHtml_(report.startedAt) +
      " | Finished: " +
      escapeHtml_(report.finishedAt) +
      "</div>" +
      "</body></html>"
    );
  }
  ...
  function objectHasNonEmptyArrays_(value) {
    ...
    for (var key in value) {
      ...
      if (
        ...
        objectHasNonEmptyArrays_(value[key])
      )
        return true;
    }
    return false;
