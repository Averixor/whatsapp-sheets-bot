...
const ROLE_METADATA = Object.freeze({
  guest: Object.freeze({
    label: "Guest",
    note: "Guest • safe read-only preview only",
  }),
  viewer: Object.freeze({ label: "Viewer", note: "Viewer • own card only" }),
  operator: Object.freeze({
    label: "Operator",
    note: "Operator • working access to cards and summaries",
  }),
  maintainer: Object.freeze({
    label: "Maintainer",
    note: "Maintainer • extended diagnostics and maintenance access",
  }),
  admin: Object.freeze({
    label: "Admin",
    note: "Admin • access, logs, and system tools management",
  }),
  sysadmin: Object.freeze({
    label: "Sysadmin",
    note: "Sysadmin • full technical maintenance, repair, and triggers",
  }),
  owner: Object.freeze({
    label: "Owner",
    note: "Owner • full root access to the entire system",
  }),
});
...
function isEnabledValue_(value) {
  const raw = String(value === "" || value === null ? "TRUE" : value)
    .trim()
    .toLowerCase();
...
function isSpreadsheetProtectionWriteError_(error) {
...
function generateAccessTemporaryPassword_(seed) {
  ...
  var sourceHash = hashTextSha256_(
    [
      "WASB_ACCESS_TEMP_PASSWORD_V1",
      String(seed || ""),
      Utilities.getUuid(),
      String(Date.now()),
      String(Math.random()),
...
function hashAccessPasswordWithSalt_(password, salt) {
  ...
  return hashTextSha256_(
    ["WASB_ACCESS_PASSWORD_V1", normalizedSalt, normalizedPassword].join("|"),
  );
}

function getAccessTemporaryPasswordExpiresAt_(hours) {
  var ttlHours =
    Number(hours || ACCESS_TEMP_PASSWORD_TTL_HOURS) ||
...
function _normalizeLoginMeta_(meta) {
  ...
  const coords =
    geo.coords && typeof geo.coords === "object" ? geo.coords : null;

  return {
    enteredAtIso: String(raw.enteredAtIso || "").trim(),
    enteredAtText:
      String(raw.enteredAtText || "").trim() || "Невідомий час входу",
    utcOffset: String(raw.utcOffset || "").trim(),
    loginPointText:
      String(raw.loginPointText || "").trim() || "Точка входу: GPS недоступна",
    geo: {
      ok: !!geo.ok,
...
function _reportSelfBindViolation_(actionName, details, descriptorOpt) {
  try {
    ...
    Logger.log(
      "[AccessControl] self-bind violation report failed: " +
...
function _registerSelfBindFailure_(currentKeyHash, context) {
  ...
  const remainingAttempts = Math.max(
    MAX_SELF_BIND_LOGIN_ATTEMPTS - nextAttempts,
    0,
  );
  const loginMeta = _normalizeLoginMeta_(context?.loginMeta);

  _reportSelfBindViolation_(
    locked ? "selfBindLoginBlocked" : "selfBindLoginDenied",
    {
  ...
  return {
    ...
    remainingAttempts: remainingAttempts,
    remainingMinutes: locked
      ? _minutesText_(SELF_BIND_LOCK_DURATION_MS)
      : publicState.remainingMinutes,
...
function _failureMessageForSelfBind_(reasonCode, callsign, failureState) {
  ...
  if (reasonCode === REASON_CODES.SELF_BIND_CALLSIGN_OCCUPIED) {
    return (
      "Цей позивний уже зайнятий. Якщо це ваш позивний — " +
  ...
  return (
    "Дані не збігаються. Перевірте email або телефон і позивний. Залишилось спроб: " +
    Math.max(Number(failureState?.remainingAttempts || 0), 0) +
    ". Якщо це ваш позивний — " +
    getSelfBindHelpText_().toLowerCase() +
    "."
  );
}
...
function formatUaLongDateTime_(value) {
  ...
  return (
    ...
    " GMT" +
    Utilities.formatDate(date, tz, "Z").replace(/(\d{2})(\d{2})$/, "$1:$2")
  );
}
