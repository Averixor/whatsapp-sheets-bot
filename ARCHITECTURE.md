...
| Setup / refresh API | Sidebar `apiStage7SetupTemporaryPropertyRegister()` / `apiStage7RefreshTemporaryPropertyRegister()` (GAS aliases `apiSetupTemporaryPropertyRegister` / `apiRefreshTemporaryPropertyRegister`) | One-time seed/migrate/backup; safe re-validate |
