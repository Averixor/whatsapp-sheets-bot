...
function checkDuplicates() {
  ...
  try {
    ...
    data.forEach(function (row, idx) {
      ...
      if (callsignCol >= 0) {
        const cs = String(row[callsignCol] || "")
          .trim()
          .toUpperCase();
        if (cs) {
    ...
    Object.entries(counters.fml).forEach(function (entry) {
      ...
      const rows = entry[1];
      if (rows.length > 1) {
        duplicates.push('ПІБ "' + value + '" → ' + rows.join(", "));
      }
    });
    ...
    Object.entries(counters.callsign).forEach(function (entry) {
      ...
      const rows = entry[1];
      if (rows.length > 1) {
        duplicates.push('Позивний "' + value + '" → ' + rows.join(", "));
      }
    });
    ...
    Object.entries(counters.phone).forEach(function (entry) {
      ...
      const rows = entry[1];
      if (rows.length > 1) {
        duplicates.push('Телефон "' + value + '" → ' + rows.join(", "));
      }
    });
    ...
    if (!duplicates.length) {
      ...
      duplicates.forEach(function (message) {
        _pushCheck_(report, {
          type: "duplicate",
          name: "duplicate",
          status: "WARN",
