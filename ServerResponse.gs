...
function mergeServerResponseTopLevelMeta_(source, targetMeta) {
  const meta =
    targetMeta && typeof targetMeta === "object"
...
function buildServerResponseEnvelope_(
  success,
  message,
  error,
  result,
  ...
  const data = buildServerResponseData_(
    result,
    changes,
    safeMeta,
    safeDiagnostics,
  ...
  return {
    ...
    operationId:
      safeMeta.operationId || (safeContext && safeContext.operationId) || null,
    scenario:
      safeMeta.scenario || (safeContext && safeContext.scenario) || null,
    ...
    appliedChangesCount: toServerResponseCount_(
      safeMeta.appliedChangesCount,
      data.changes.length,
    ),
    skippedChangesCount: toServerResponseCount_(
      safeMeta.skippedChangesCount,
      0,
    ),
...
function buildSimpleServerResponse_(
  success,
  data,
  message,
  error,
  ...
  if (
    ...
    changes = Array.isArray(data.changes)
      ? data.changes
      : serverResponseAsArray_(data.changes);
    meta =
...
function okResponse_(data, message, context, warnings) {
  return buildSimpleServerResponse_(
    true,
    data,
    message || "",
    null,
...
function normalizeServerResponse_(value, functionName, context) {
  ...
  if (
    ...
    if (
      ...
      changes = Array.isArray(rawData.changes)
        ? rawData.changes
        : serverResponseAsArray_(rawData.changes);
      meta =
        rawData.meta && typeof rawData.meta === "object"
          ? Object.assign({}, rawData.meta)
          : {};
  ...
  if (
    ...
    return errorResponse_(
      data.error || "Операція не виконана",
      baseContext,
      data,
    );
  }
  ...
  if (typeof value === "boolean") {
    return buildSimpleServerResponse_(
      ...
      { value: value },
      "",
      value ? null : "Операція повернула false",
      baseContext,
      [],
    );
  }
...
function withResponseContext_(response, extraContext) {
  ...
  normalized.context = Object.assign(
    {},
    normalized.context || {},
    extraContext || {},
  );
...
function ensureApiSuccess_(response, fallbackMessage) {
  ...
  if (!normalized.success) {
    throw buildContextError_(
      normalized.context && normalized.context.function
        ? normalized.context.function
        : "ensureApiSuccess_",
      normalized.context,
      normalized.error ||
        fallbackMessage ||
        "Server-side response is not successful",
    );
  }
  return normalized;
}
