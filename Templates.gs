...
function renderTemplate_(tpl, data) {
  ...
  return tpl.replace(
    ...
    function (m, keyDouble, keySingle) {
      const key = keyDouble || keySingle || "";
      const v =
        data[key] ??
        data[String(key).toLowerCase()] ??
        data[String(key).toUpperCase()];
      return v === undefined || v === null ? "" : String(v);
    },
  );
}

/** Normalize ENABLED: checkbox boolean or "TRUE"/"1"/"YES" */
...
/** Load template map from sheet (and store in Cache) */

function _loadTemplatesMap_() {
  const cache = CacheService.getScriptCache();
  const primaryKey = _templatesCacheKey_();
  const cacheKeys = [primaryKey, TPL_CACHE_KEY].filter(
    ...
    try {
      ...
      if (parsed && typeof parsed === "object") {
        if (cacheKeys[i] !== primaryKey) {
          try {
            cache.put(primaryKey, cached, TPL_CACHE_TTL_SEC);
          } catch (e) {}
        }
        return parsed;
      }
...
/** Template text: return '' when ENABLED=false or empty */
...
/**
 * Build WhatsApp Web link from TEMPLATES text.
 * Returns link or null when template is disabled/empty or phone is missing.
 */
function notifyWithTemplate_(templateKey, data, targetPhone) {
  templateKey = String(templateKey || "").trim();
  if (!templateKey) throw new Error("templateKey is empty");

  const tplText = getTemplateText_(templateKey);

  if (!tplText) {
    console.warn(
      `[SKIP] Template "${templateKey}" disabled (ENABLED=false / empty / not found).`,
    );
    return null;
  }
  ...
  if (!digits) {
    console.warn(
      `[SKIP] Template "${templateKey}" no valid targetPhone: "${phone}"`,
    );
    return null;
  }

  const safeMsg =
    typeof trimToEncoded_ === "function" &&
...
/** Clear template cache (after TEMPLATES sheet edits) */
...
function testNotifyWithTemplate_() {
  ...
  if (
    link &&
    link.includes("web.whatsapp.com/send") &&
    link.includes("text=")
  ) {
...
const Stage7Templates_ = (function () {
  ...
  function listKeys() {
    ...
    return [
      ...new Set(
        Object.keys(STAGE7_INLINE_TEMPLATES_).concat(
