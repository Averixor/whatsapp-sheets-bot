...
| Inventory | `inventory/` | `InventoryReconciliation.gs`, `TemporaryPropertyRegister.gs` | `npm run ci` (`verify-clasp-push-patterns.mjs` imports `verify-inventory-reconciliation.mjs`); `npm run ci:workbook` for temporary-property | Sidebar **Звірка** plus temporary-property setup/refresh on sidebar (not top menu); sheets `Property_issued_for_temporary_u`, `PROPERTY_CATALOG` / `PROPERTY_KITS`. |
...
| Diagnostics | `diagnostics/` | `Diagnostics.*.gs`, `SystemStatus.Foundation.gs`, `SystemStatus.Probes.gs`, `SystemStatus.Fingerprints.gs` | `npm run ci` (`verify-system-status-foundation.mjs`, `verify-system-status-fingerprints.mjs`) | Foundation + fingerprints (SS-1/SS-2); public API/routing later (SS-3). Tests: `tests/SystemStatus*.gs`. Stage7TestRunner still deployed with clasp. |
