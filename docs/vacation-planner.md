...
## Monthly schedule sync

`VacationMonthlySync_` is the only mutating bridge from approved/applied
vacation records into monthly sheets.

- `apiStage7CreateNextMonth()` / `createNextMonthSheet()` run the sync after the
  new month is copied, dates are reset, and monthly callsigns are refreshed.
- `apiStage7MaterializeComputedData()` also runs the sync for the active or
  requested month.
- Empty fact cells are filled with the allowed monthly vacation code (`Відпус`
  when the dropdown allows it).
- Non-empty cells with a different value become sidebar conflicts; the sync does
  not overwrite them until a maintainer confirms the pending plan.
- Confirmed replacements append a service note with previous value, new value,
  source row, and decision metadata. Sync metadata is also stored in Document
  Properties so moved/cancelled source vacations can be detected later.
- Moved/cancelled vacations only become pending removals when metadata proves
  the monthly cell was generated by the sync and the current value still matches
  that synced value.
- The sync validates existing dropdown values and does not add new monthly codes.
...
text in the footer summary. `scripts/verify-vacation-monthly-sync.mjs` covers
auto-fill, idempotency, conflict confirmation notes, unsupported dropdowns, and
metadata-backed removals.
