...
function loadPhonesIndex_() {
  ...
  var birthdayCol = _stage7ShimFindHeaderCol_(
    headers,
    [
      "birthday",
      "день народження",
...
function findPhone_(query, options) {
  ...
  function addCandidate_(list, value) {
    ...
    if (typeof normalizeFML_ === "function") {
      try {
        list.push(normalizeFML_(value));
      } catch (_) {}
    }

    if (typeof _normFmlForProfiles_ === "function") {
      try {
        list.push(_normFmlForProfiles_(value));
      } catch (_) {}
    }

    if (typeof _normCallsignKey_ === "function") {
      try {
        list.push(_normCallsignKey_(value));
      } catch (_) {}
    }
  }
  ...
  if (directPhone) {
    if (typeof normalizePhone_ === "function") {
      try {
        return normalizePhone_(directPhone);
      } catch (_) {}
    }
  ...
  for (var r = 0; r < rows.length; r++) {
    ...
    addCandidate_(
      rowKeys,
      row.fml || row.fio || row.fullName || row.name || "",
    );
...
function loadDictMap_() {
  ...
  var labelCol = _stage7ShimFindHeaderCol_(
    headers,
    ["label", "назва", "послуга", "service"],
    1,
  );
  var placeCol = _stage7ShimFindHeaderCol_(
    headers,
    ["place", "місце", "location"],
    2,
  );
  var taskCol = _stage7ShimFindHeaderCol_(
    headers,
    ["task", "завдання", "message", "повідом"],
    3,
  );
...
function buildPayloadForCell_(
  ...
  function normalizeDateCompat_(cell) {
    ...
    try {
      display = clean_(cell.getDisplayValue());
    } catch (_) {}
  ...
  if (isSheetCall) {
    ...
    try {
      callsignFromSheet = clean_(
        sheet.getRange(row, callsignCol).getDisplayValue(),
      );
    } catch (_) {}
  ...
  var fml = clean_(
    input.fml || input.fio || input.fullName || input.name || "",
  );
  ...
  var service = clean_(
    input.service || dictItem.service || dictItem.label || code,
  );
  ...
  var brDaysOut =
    input.brDays === undefined || input.brDays === null ? 0 : input.brDays;
