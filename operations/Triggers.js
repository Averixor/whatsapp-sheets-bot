...
const Stage7Triggers_ = (function () {
  ...
  function installManagedTriggers() {
    ...
    try {
      PropertiesService.getDocumentProperties().setProperty(
        "STAGE7:MANAGED_TRIGGERS_INSTALLED_AT",
  ...
  function _buildRuntimeContext(registryItem, options) {
    ...
    return {
      ...
      initiatorRole: String(
        opts.initiatorRole || descriptor.role || (opts.trigger ? "system" : ""),
      ),
      ...
      entryPoint: String(
        opts.entryPoint ||
          (opts.trigger ? registryItem.handler : "Stage7Triggers_.runJob"),
      ),
      triggerId: String(opts.triggerId || ""),
      notes: String(
        opts.notes ||
          (opts.trigger
            ? "trigger:" + registryItem.handler
            : "manual job launch"),
      ),
    };
  }
  ...
  function _executeJob(registryItem, options) {
    ...
    switch (registryItem.jobName) {
      case _cfg().JOBS.DAILY_VACATIONS_AND_BIRTHDAYS:
        return Stage7UseCases_.checkVacationsAndBirthdays(
          Object.assign({}, opts, { date: _todayStr_() }),
        );
      case _cfg().JOBS.SCHEDULED_RECONCILIATION:
        return Stage7UseCases_.runReconciliation(
          Object.assign({}, opts, {
            mode: "report",
            date: _todayStr_(),
            dryRun: true,
      ...
      case _cfg().JOBS.SCHEDULED_HEALTHCHECK:
        return Stage7UseCases_.runMaintenanceScenario(
          Object.assign({}, opts, { type: "healthCheck", shallow: true }),
        );
      case _cfg().JOBS.CLEANUP_CACHES:
        return Stage7UseCases_.runMaintenanceScenario(
          Object.assign({}, opts, { type: "cleanupCaches" }),
        );
      case _cfg().JOBS.STALE_OPERATION_DETECTOR:
        return typeof OperationRepository_ === "object"
          ? {
              success: true,
              message: "Stale detector виконано",
              result: OperationRepository_.detectStaleOperations(),
            }
          : {
              success: false,
              message: "OperationRepository_ недоступний",
              result: { staleOperations: [] },
            };
      case _cfg().JOBS.LIFECYCLE_RETENTION_CLEANUP:
        return Stage7UseCases_.runMaintenanceScenario(
          Object.assign({}, opts, { type: "cleanupLifecycleRetention" }),
        );
      default:
        throw new Error(
          `Stage7 job "${registryItem.jobName}" не підтримується`,
        );
    }
  }
  ...
  function runJob(jobName, options) {
    ...
    const registryItem = Object.keys(REGISTRY)
      ...
      .find(function (item) {
        return item.jobName === name || item.handler === name;
      });
    ...
    return JobRuntime_.observe(
      registryItem.jobName,
      _buildRuntimeContext(registryItem, opts),
      function () {
        return _executeJob(registryItem, opts);
      },
    );
