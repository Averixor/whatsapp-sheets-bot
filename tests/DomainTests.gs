    ...
    getRange: function (row, col, numRows, numCols) {
      ...
      return {
        ...
        getDisplayValue: function () {
          return String(
            rangeValues[0] && rangeValues[0][0] !== undefined
              ? rangeValues[0][0]
              : "",
          );
        },
        getValue: function () {
          return rangeValues[0] && rangeValues[0][0] !== undefined
            ? rangeValues[0][0]
            : "";
        },
        getNumRows: function () {
...
function _runPersonnelRepositoryDomainTests_(report) {
  _domainPush_(
    report,
    "personnel.status normalization canonical and legacy",
    function () {
      [
        ["", PERSONNEL_STATUS_AVAILABLE_UA_],
        [null, PERSONNEL_STATUS_AVAILABLE_UA_],
        ...
        [PERSONNEL_STATUS_HOSPITAL_UA_, PERSONNEL_STATUS_HOSPITAL_UA_],
        [PERSONNEL_STATUS_SZCH_UA_, PERSONNEL_STATUS_SZCH_UA_],
        [PERSONNEL_STATUS_BZVP_UA_, PERSONNEL_STATUS_BZVP_UA_],
      ].forEach(function (pair) {
        _domainAssertEqual_(
          normalizePersonnelStatus_(pair[0]),
          pair[1],
          "normalizePersonnelStatus_ mismatch",
        );
      });

      _domainAssert_(
        isPersonnelStatusActive_("") === true,
        "Порожній Status має бути активним В наявності",
      );
      _domainAssert_(
        isPersonnelStatusActive_("Temp") === true,
        "Legacy Temp має бути активним",
      );
      _domainAssert_(
        isPersonnelStatusActive_("Removed") === false,
        "Legacy Removed має бути неактивним",
      );
      _domainAssert_(
        isPersonnelStatusActive_(PERSONNEL_STATUS_SZCH_UA_) === false,
        "СЗЧ має бути неактивним",
      );
      _domainAssert_(
        isPersonnelStatusActive_(PERSONNEL_STATUS_HOSPITAL_UA_) === true,
        "Лікарняний має бути активним",
      );
      _domainAssertEqual_(
        getPersonnelStatusCanonical_(PERSONNEL_STATUS_REMOVED_UA_),
        "Removed",
        "Вибув canonical",
      );
      _domainAssertEqual_(
        getPersonnelStatusListValues_().length,
        9,
        "dropdown count",
      );
      _domainAssertEqual_(
        getPersonnelStatusListValues_()[0],
        PERSONNEL_STATUS_AVAILABLE_UA_,
        "dropdown order",
      );
  ...
  _domainPush_(
    report,
    "personnel.header aliases optional ID and rank fallback",
    function () {
      const headers = [
        "ПІБ",
        "День народження",
      ...
      _domainAssertEqual_(
        ...
        "Status normalization in record",
      );
      _domainAssert_(
        record.active === false,
        "Removed/Вибув record must be inactive",
      );
...
function _runMonthlyLayoutDomainTests_(report) {
  _domainPush_(
    report,
    "monthly.layout detects standard sheet geometry",
    function () {
  ...
  _domainPush_(
    report,
    "monthly.layout detects compact sheet geometry",
    function () {
      const sheet = _domainMakeFakeSheet_("06", [
  ...
  _domainPush_(
    report,
    "monthly.layout rejects unsupported headers",
    function () {
      const sheet = _domainMakeFakeSheet_("06", [
...
function runStage6ADomainTests_(options) {
  ...
  _domainPush_(
    report,
    "templates.renderTemplate basic substitution",
    function () {
  ...
  _domainPush_(report, "templates.renderTemplate double braces", function () {
    ...
    _domainAssert_(
      out === "Привіт, Сергій!",
      "Подвійні дужки {{name}} не працюють",
    );
    return out;
  });
  ...
  _domainPush_(report, "templates.missing keys detection", function () {
    ...
    _domainAssert_(
      resolved.missingKeys.indexOf("date") !== -1,
      "missingKeys не містить date",
    );
  ...
  _domainPush_(report, "templates.fallback resolution", function () {
    ...
    _domainAssert_(
      descriptor.source === "system-fallback" ||
        descriptor.source === "managed-sheet",
      "Не знайдено fallback/managed template",
    );
  ...
  _domainPush_(report, "templates.preview vs final mode", function () {
    ...
    const preview = Stage7Templates_.preview("DAY_SUMMARY_HEADER", data, {
      maxLen: 5,
    });
    const full = Stage7Templates_.render("DAY_SUMMARY_HEADER", data, {
      preview: false,
    });
    _domainAssert_(preview.length <= 6, "Preview не обрізається");
    _domainAssert_(
      full.indexOf("01.01.2026") !== -1,
      "Final render пошкоджений",
    );
  ...
  _domainPush_(
    report,
    "sendPanel.key generation strips formatting",
    function () {
  ...
  _domainPush_(report, "phone lookup canonical index contract", function () {
    ...
    _domainAssert_(
      byRole === "+380662222222",
      "findPhone_() не знайшов телефон по role",
    );
    _domainAssert_(
      byCallsign === "+380663333333",
      "findPhone_() не знайшов телефон по callsign",
    );
  ...
  _domainPush_(report, "sendPanel.normalize rows", function () {
    ...
    _domainAssert_(
      rows.length === 1,
      "normalizeRows повинен відкидати порожні рядки",
    );
    _domainAssert_(
      rows[0].phone === "+380661234567",
      "Телефон не нормалізовано",
    );
  ...
  _domainPush_(report, "sendPanel.duplicate detection", function () {
    ...
    _domainAssert_(
      duplicates.length === 1,
      "Duplicate detection повинен знайти 1 ключ",
    );
    return duplicates[0];
  });

  _domainPush_(report, "sendPanel.sent transition rules", function () {
    const next = SendPanelService_.resolveTransition(
      { sent: false, status: getSendPanelReadyStatus_() },
      "markSent",
    );
    _domainAssert_(
      next.sent === true && next.status === getSendPanelReadyStatus_(),
      "markSent rule пошкоджений",
    );
    return next.status;
  });
  ...
  _domainPush_(report, "sendPanel.pending transition rules", function () {
    ...
    _domainAssert_(
      next.sent === false && next.status === getSendPanelReadyStatus_(),
      "markPending rule пошкоджений",
    );
    return next.status;
  });
  ...
  _domainPush_(report, "sendPanel.unsent transition rules", function () {
    const next = SendPanelService_.resolveTransition(
      { sent: true, status: getSendPanelReadyStatus_() },
      "markUnsent",
    );
    _domainAssert_(
      next.sent === false && next.status === getSendPanelReadyStatus_(),
      "markUnsent rule пошкоджений",
    );
    return next.status;
  });
  ...
  _domainPush_(report, "sendPanel.allowed statuses canonical set", function () {
    ...
    _domainAssert_(
      statuses.indexOf(SendPanelConstants_.STATUS_READY) !== -1,
      "Немає STATUS_READY",
    );
    _domainAssert_(
      statuses.indexOf(SendPanelConstants_.STATUS_READY) !== -1,
    ...
    _domainAssert_(
      statuses.indexOf(SendPanelConstants_.STATUS_BLOCKED) !== -1,
      "Немає STATUS_BLOCKED",
    );
  ...
  _domainPush_(report, "reconciliation.compare missing rows", function () {
    ...
    const types = result.issues.map(function (item) {
      return item.type;
    });
    _domainAssert_(
      types.indexOf("missingExpectedItem") !== -1,
      "Не знайдено missingExpectedItem",
    );
    return types.join(",");
  });
  ...
  _domainPush_(report, "reconciliation.compare extra rows", function () {
    ...
    _domainAssert_(
      types.indexOf("orphanSendPanelRow") !== -1,
      "Не знайдено orphanSendPanelRow",
    );
    return types.join(",");
  });
  ...
  _domainPush_(report, "reconciliation.compare duplicates", function () {
    ...
    _domainAssert_(
      types.indexOf("duplicateSendPanelRow") !== -1,
      "Не знайдено duplicateSendPanelRow",
    );
    return types.join(",");
  });
  ...
  _domainPush_(report, "reconciliation.compare stale statuses", function () {
    ...
    _domainAssert_(
      types.indexOf("staleStatus") !== -1,
      "Не знайдено staleStatus",
    );
    return types.join(",");
  });
  ...
  _domainPush_(report, "reconciliation.targeted repair preview", function () {
    ...
    _domainAssert_(
      ...
      "previewRepairPlan некоректно фільтрує issueTypes",
    );
  ...
  _domainPush_(
    report,
    "reconciliation.post-repair verification formatter",
    function () {
      ...
      _domainAssert_(
        verification.remainingIssues === 1,
        "remainingIssues має бути 1",
      );
      _domainAssert_(
        verification.criticalRemaining === 0,
        "criticalRemaining має бути 0",
      );
  ...
  _domainPush_(report, "date.normalize valid formats", function () {
    ...
    _domainAssert_(
      norm === "17.03.2026",
      "normalizeDate не нормалізував YYYY-MM-DD",
    );
    return norm;
  });
  ...
  _domainPush_(report, "date.normalize invalid throws", function () {
    ...
    _domainAssert_(
      thrown,
      "normalizeDate повинен кидати помилку на не існуючу дату",
    );
    return "throws-ok";
  });
  ...
  _domainPush_(
    report,
    "sheetSchema.monthly compact layout detection",
    function () {
      ...
      _domainAssert_(
        ...
        "Compact monthly layout не визначено",
      );
      _domainAssert_(
        layout.codeRangeA1 === "C2:D3",
        "Compact codeRangeA1 має бути C2:D3",
      );
      _domainAssert_(
        getMonthlyCallsignColForSheet_(sheet) === 2,
        "Compact Callsign має бути у колонці B",
      );
      _domainAssert_(
        getMonthlyBrDaysColForSheet_(sheet) === 1,
        "Compact BR days має бути у колонці A",
      );
      return layout.codeRangeA1;
    },
  );
  ...
  _domainPush_(
    report,
    "sheetSchema.monthly standard layout detection",
    function () {
      const sheet = makeDomainFakeSheet_([
        [
          "Телефон",
      ...
      _domainAssert_(
        ...
        "Standard monthly layout не визначено",
      );
      _domainAssert_(
        layout.codeRangeA1 === "H2:I3",
        "Standard codeRangeA1 має бути H2:I3",
      );
      _domainAssert_(
        getMonthlyCallsignColForSheet_(sheet) === 2,
        "Standard Callsign має бути у колонці B",
      );
      _domainAssert_(
        getMonthlyBrDaysColForSheet_(sheet) === 6,
        "Standard BR days має бути у колонці F",
      );
      return layout.codeRangeA1;
    },
  );
  ...
  _domainPush_(report, "summary.day payload message builder", function () {
    ...
    _domainAssert_(
      msg.indexOf("17.03.2026") !== -1,
      "Повідомлення не містить дату",
    );
    _domainAssert_(
      msg.indexOf("Охорона") !== -1,
      "Повідомлення не містить service",
    );
  ...
  _domainPush_(report, "summary.detailed composition", function () {
    ...
    _domainAssert_(
      text.indexOf("17.03.2026") !== -1,
      "Detailed summary не містить дату",
    );
    _domainAssert_(
      text.indexOf("Петренко") !== -1,
      "Detailed summary не містить прізвище",
    );
  ...
  _domainPush_(report, "summary.empty-state behavior", function () {
    ...
    _domainAssert_(
      text.indexOf("17.03.2026") !== -1,
      "Навіть empty state має містити дату",
    );
    return "len=" + text.length;
  });
  ...
  _domainPush_(report, "summary.optional fields behavior", function () {
    ...
    _domainAssert_(
      msg.indexOf("17.03.2026") !== -1,
      "Minimal message не містить дату",
    );
    return "len=" + msg.length;
  });
  ...
  _domainPush_(report, "routing.route resolution", function () {
    ...
    _domainAssert_(
      route && route.routeName === "sidebar.generateSendPanelForDate",
      "Route resolution by API method зламана",
    );
  ...
  _domainPush_(report, "routing.action normalization", function () {
    ...
    _domainAssert_(
      normalized === "sidebar.generateSendPanelForDate",
      "UI action normalization зламана",
    );
    return normalized;
  });
  ...
  _domainPush_(
    report,
    "contract normalization / top-level fields",
    function () {
      const response = buildServerResponse_(
        ...
        { lifecycle: ["response.built"] },
        { scenario: "x" },
        [],
      );
      _domainAssert_(
        response.operationId === "op1",
        "operationId має дублюватися у top-level",
      );
  ...
  _domainPush_(report, "contract.partial success formatting", function () {
    ...
    _domainAssert_(
      response.skippedChangesCount === 2,
      "skippedChangesCount має бути 2",
    );
    return "partial-ok";
  });

  report.total = report.checks.length;
  report.passed = report.checks.filter(function (item) {
    return item.status === "OK";
  }).length;
  report.failed = report.checks.filter(function (item) {
