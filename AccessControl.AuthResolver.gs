...
function loginByIdentifierAndCallsign(
  ...
  return _errorResponse(
    "access.self_bind.use_access_requests",
    "Самостійний вхід тимчасово вимкнено. Натисніть «Отримати ключ доступу» або зверніться до адміністратора.",
    supportCallsign,
    loginMeta,
  );
}
...
function submitAccessKeyRequest(payload) {
  ...
  try {
    if (
      ...
      typeof appendAccessRequest_ !== "function"
    ) {
      return {
        success: false,
        code: "access.registration.unavailable",
        message:
          "Модуль ACCESS_REQUESTS недоступний. Зверніться до адміністратора.",
      };
    }
    ...
    const displayName =
      ...
      callsign ||
      login;
    const appendResult = appendAccessRequest_({
      request_type: "access_request",
      status: "pending",
      source: "sidebar_key_request",
      user_email: email,
      phone: phone,
      login: login,
      person_callsign: callsign,
      surname: surname,
      ...
      request_user_key_hash_masked: maskSensitiveValue_(currentKeyHash),
    });

    if (appendResult.duplicate) {
      return {
        success: true,
        code: "access.registration.duplicate_pending",
        message:
          appendResult.message ||
          "Заявка вже існує. Очікуйте підтвердження адміністратора.",
        requestId:
          appendResult.request && appendResult.request.request_id
            ? appendResult.request.request_id
            : "",
      };
    }

    const savedRequest = appendResult.request || {};
    const details = {
      reasonCode: "access.registration.requested",
      reasonMessage:
        "Користувач подав заявку на отримання ключа доступу (ACCESS_REQUESTS).",
      email: email,
      phone: phone,
      login: login,
      ...
      accessRequestsSheetRow: savedRequest.sheetRow || "",
    };
...
function registerAccessWithTemporaryPassword(payload) {
  ...
  try {
    ...
    if (
      typeof canWriteProtectedAccess_ === "function" &&
    ...
    const activationRequestedAt = nowText;
    const activationProof = buildAccessActivationProof_({
      requestId: requestId,
      requestUserKeyHash: currentKeyHash,
      accessRow: entry.sheetRow,
      temporaryPasswordHash: entry.temporaryPasswordHash,
      activationPasswordHash: passwordHash,
      activationPasswordSalt: passwordSalt,
      activationRequestedAt: activationRequestedAt,
    });

    const appendResult = appendAccessRequest_({
      request_id: requestId,
      request_type: "activation_request",
      status: "activation_pending",
      source: "sidebar_activation",
      request_user_key_hash: currentKeyHash,
      request_user_key_hash_masked: maskSensitiveValue_(currentKeyHash),
      activation_login: login,
      activation_password_hash: passwordHash,
      activation_password_salt: passwordSalt,
      activation_proof: activationProof,
      activation_requested_at: activationRequestedAt,
      access_row: entry.sheetRow,
    });

    if (appendResult.duplicate) {
      return {
        success: true,
        code: "access.registration.activation_pending",
        message: "Активацію вже прийнято. Оновіть панель через кілька секунд.",
        requestId:
          appendResult.request && appendResult.request.request_id
            ? appendResult.request.request_id
            : "",
    ...
    if (typeof processAccessRequestsQueue_ === "function") {
      try {
        queueResult = processAccessRequestsQueue_({
          requestId: requestId,
          requireAdmin: false,
        });
      } catch (_) {}
    ...
    var activated =
      queueResult &&
      Array.isArray(queueResult.processed) &&
      queueResult.processed.some(function (item) {
    ...
    return {
      success: true,
      code: "access.registration.activation_queued",
      message:
        "Активацію прийнято. Оновіть панель через хвилину або зверніться до адміністратора.",
      requestId: requestId,
      accessSheetRow: entry.sheetRow,
      queued: true,
    };
